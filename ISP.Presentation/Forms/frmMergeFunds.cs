...
namespace ISP.Presentation.Forms
{
    public partial class frmMergeFunds : Form, IMessageFilter
    {
        ...
        public frmMergeFunds(Presentation.Forms.frmMain mf, Fund fundRecord1, Fund fundRecord2)
        {
            ...
            if (fund1.AdvisorId != null)
            {
                Advisors advisor = new Advisors((Guid)fund1.AdvisorId);
                txtFund1Advisor.Text = advisor.Name;
            }

            if (fund2.AdvisorId != null)
            {
                Advisors advisor = new Advisors((Guid)fund2.AdvisorId);
                txtFund2Advisor.Text = advisor.Name;
            }
            ...
            if (!String.IsNullOrWhiteSpace(fund1.InceptDate.ToString()))
                txtFund1InceptDate.Text = ((DateTime)fund1.InceptDate).ToString("MMMM dd, yyyy");
            ...
            if (!String.IsNullOrWhiteSpace(fund2.InceptDate.ToString()))
                txtFund2InceptDate.Text = ((DateTime)fund2.InceptDate).ToString("MMMM dd, yyyy");

            Fund.DataMaintenance fund1DataMantenance = new Fund.DataMaintenance(fund1.Id);
            ...
            Fund.DataMaintenance fund2DataMantenance = new Fund.DataMaintenance(fund2.Id);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Guid masterFundId = ((Fund)((ListItem)cboMasterRecord.SelectedItem).HiddenObject).Id;
            Guid minorFundId;

            if (masterFundId == fund1.Id)
                minorFundId = fund2.Id;
            else
                minorFundId = fund1.Id;
